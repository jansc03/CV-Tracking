...
class DualBackgroundSubtraction:
    ...
    backSub2 = cv2.createBackgroundSubtractorMOG2(history=200, detectShadows=True, varThreshold=150)
    ...
    def get_mask(self):
        ...
        # Verbesserungen der Maske
        _, mask1 = cv2.threshold(mask1, 50, 255, cv2.THRESH_BINARY)
        kernel = cv2.getStructuringElement(cv2.MORPH_ELLIPSE, (5, 5))
        mask1 = cv2.morphologyEx(mask1, cv2.MORPH_CLOSE, kernel)  # Schließen von Löchern
        mask1 = cv2.morphologyEx(mask1, cv2.MORPH_OPEN, kernel)  # Entfernen von Rauschen
